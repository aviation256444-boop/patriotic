...
function SuccessContent() {
  ...
  const rows = [
    ...(receipt.campaign
      ...
      : []),
    ...(receipt.phone ? [{ label: "MoMo number", value: receipt.phone }] : []),
    ...(receipt.paymentId
      ? [{ label: "Payment ID", value: receipt.paymentId.slice(0, 18) + "…" }]
      : []),
  ];
  ...
  return (
    <OfficialReceipt
      ...
      subtitle="Your donation to Patriotic Youths of Uganda was confirmed successfully. This single-page receipt is valid for your records."
      amount={Number(receipt.amount)}
      currency={receipt.currency || "UGX"}
      method={gatewayLabel(receipt.gateway)}
      reference={receipt.externalId || receipt.paymentId}
      paidAt={receipt.paidAt}
      statusLabel="Donation received"
      rows={rows}
      secondaryHref="/donate"
      secondaryLabel="Donate again"
    />
  );
}
      ...
      <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 print:max-w-none print:px-0">
        <div className="print-hide-hero no-print mb-8 text-center space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            Designed for one-page printing · Save as PDF from your browser
          </p>
        </div>
